...
export default function NewsListClient({ isAdmin }: { isAdmin: boolean }) {
  ...
  // Çoklu seçim + toplu işlem durumu.
  ...
  // Filtre/sayfa değişince seçimi temizle (görünmeyen satırlara işlem yapılmasın).
  useEffect(() => {
    setSelected(new Set());
  ...
  async function runBulk(
    action: BulkAction,
    extra?: { category?: NewsCategory; sport?: string },
  ) {
    const ids = [...selected];
    ...
    if (
      action === "ARCHIVE" &&
      !confirm(`${ids.length} haberi arşivlemek istediğinize emin misiniz?`)
    )
      return;
    setBulkBusy(true);
    try {
  ...
  return (
    <div className="stack">
      ...
      {/* Toplu işlem çubuğu — en az bir satır seçiliyken görünür (yüzer). */}
      {selected.size > 0 && (
        <div className="bulk-bar">
          ...
          <div className="bulk-actions">
            ...
            <select
              ...
              value=""
              onChange={(e) => {
                const v = e.target.value as NewsCategory;
                if (v) void runBulk("SET_CATEGORY", { category: v });
            ...
            <select
              ...
              onChange={(e) => {
                ...
                if (v) void runBulk("SET_SPORT", { sport: v });
                e.currentTarget.value = "";
              }}
            >
          ...
          <button
            className="btn btn-sm btn-ghost bulk-clear"
            disabled={bulkBusy}
